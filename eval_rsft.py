#!/usr/bin/env python3
"""Evaluate RSFT checkpoint on OOD-1000.

Merges LoRA adapter, serves with vLLM, evaluates greedy on OOD-1000,
and runs paired McNemar test against baseline.

Usage:
    python eval_rsft.py [--tp 2]
"""
...
LORA_PATH = "/mnt/data/rft_checkpoints/qwen14b-rsft-a01"
MERGED_PATH = "/mnt/scratch/qwen14b_rsft/merged_model"
...
def main():
    parser = argparse.ArgumentParser(description="RSFT Evaluation")
    ...
    print(f"=== RSFT Evaluation: {datetime.now(timezone.utc).isoformat()} ===")
    print(f"  Model: {MERGED_PATH}")
    print(f"  OOD-1000: {len(problems)} problems\n")

    # Phase 1: Merge LoRA
    if not args.skip_merge:
        merge_lora(BASE_MODEL, LORA_PATH, MERGED_PATH)
    ...
    # Phase 3: Evaluate RSFT
    rsft_path = OUT_DIR / "rsft_ood1000.jsonl"
    print(f"\n--- Evaluating RSFT on OOD-1000 ---")
    proc = start_vllm_server(MERGED_PATH, tp=args.tp)
    if not wait_for_server(base_url):
        stop_server(proc)
        sys.exit(1)
    rsft_results = eval_greedy(base_url, MERGED_PATH, problems, "rsft")
    with open(rsft_path, "w") as f:
        for r in rsft_results:
            f.write(json.dumps(r, ensure_ascii=False) + "\n")
    stop_server(proc)

    # Phase 4: Paired comparison
    base_by_hash = {r["problem_hash"]: r for r in baseline_results}
    rsft_by_hash = {r["problem_hash"]: r for r in rsft_results}
    common = sorted(set(base_by_hash.keys()) & set(rsft_by_hash.keys()))

    y_base = np.array([base_by_hash[h]["correct"] for h in common])
    y_rsft = np.array([rsft_by_hash[h]["correct"] for h in common])

    stats = compute_paired_stats(y_base, y_rsft, "RSFT vs Baseline on OOD-1000")

    print(f"\n{'='*60}")
    print(f"RSFT EVALUATION RESULTS")
    print(f"{'='*60}")
    print(f"  Baseline: {stats['acc_base']*100:.1f}% ({int(y_base.sum())}/{len(common)})")
    print(f"  RSFT:     {stats['acc_new']*100:.1f}% ({int(y_rsft.sum())}/{len(common)})")
    ...
    print(f"  b (base✓ RSFT✗): {stats['b_discordant']}")
    print(f"  c (base✗ RSFT✓): {stats['c_discordant']}")
    ...
    with open(OUT_DIR / "rsft_eval_summary.json", "w") as f:
        json.dump(summary, f, indent=2)
    print(f"\n  Summary saved: {OUT_DIR / 'rsft_eval_summary.json'}")
